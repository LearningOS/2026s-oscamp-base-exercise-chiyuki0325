...
impl FlagChannel {
    ...
    pub fn produce(&self, value: u32) {
        ...
        self.ready.store(true, Ordering::Release);    // 前面的写入到不了后面
    }
    ...
    pub fn consume(&self) -> u32 {
        ...
        while !self.ready.load(Ordering::Acquire) {   // 后面的读取到不了前面
            core::hint::spin_loop();
        };
...
impl OnceCell {
    ...
    pub fn init(&self, val: u32) -> bool {
        ...
        return self.initialized.compare_exchange(false, true, Ordering::Release, Ordering::Relaxed).is_ok();
    }
